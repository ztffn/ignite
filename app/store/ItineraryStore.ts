...
export const createItinerarySlice: StateCreator<RootStore, [], [], ItineraryStore> = (set, get) => ({
  items: [],
  selectedDate: 1,
  isLoading: false,
  error: null,
  ...
  moveItem: (itemId, newDay, newTimeSlot, newPosition) => set((state) => ({
    items: state.items.map(item => 
      item.id === itemId 
        ? { 
            ...item, 
  ...
  reorderItems: (day, timeSlot, itemIds) => set((state) => {
