...
const ActivityRow: React.FC<ActivityRowProps> = ({ time, title, location, icon, color, countdown, isUrgent }) => (
  <TouchableOpacity style={{
    flexDirection: 'row',
    alignItems: 'center',
    gap: 12,
...
const QuickAction: React.FC<QuickActionProps> = ({ icon, title, subtitle, color }) => (
  <TouchableOpacity style={{
    ...
    <View style={{ flex: 1 }}>
      <Text style={{ fontSize: 14, fontWeight: '600', marginBottom: 2 }}>
        {title}
      </Text>
  ...
  return (
    <Screen preset="scroll" style={{ backgroundColor: '#f9fafb' }}>
      ...
      <View style={[$topContainerInsets, { 
        ...
        {headerExpanded && (
          <Image
            source={{ uri: 'https://images.unsplash.com/photo-1502602898536-47ad22581b52?w=800&h=200&fit=crop' }}
            style={{
              position: 'absolute',
              top: 0,
              left: 0,
