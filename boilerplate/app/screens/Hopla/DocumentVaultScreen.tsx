...
export const DocumentVaultScreen = () => {
  const { documents, searchQuery, setSearchQuery, selectedCategory, setSelectedCategory } =
    useDocumentStore()
  ...
  return (
    <Screen preset="scroll" contentContainerStyle={{ padding: 20 }}>
      <View style={[$topContainerInsets, { marginBottom: 30 }]}>
        <Text style={{ fontSize: 28, fontWeight: "bold", marginBottom: 10 }}>Document Vault</Text>
      ...
      <Card style={{ marginBottom: 20 }}>
        <View style={{ padding: 20 }}>
          <Text style={{ fontSize: 18, fontWeight: "bold", marginBottom: 15 }}>Categories</Text>
        ...
        <Card style={{ marginBottom: 20 }}>
          <View style={{ padding: 20, alignItems: "center" }}>
            ...
            <TouchableOpacity style={{ padding: 15, backgroundColor: "#007AFF", borderRadius: 8 }}>
              <Text style={{ color: "white", fontWeight: "600" }}>Upload Document</Text>
            </TouchableOpacity>
          </View>
        </Card>
      ) : (
        filteredDocuments.map((doc) => (
          <Card key={doc.id} style={{ marginBottom: 15 }}>
            <View style={{ padding: 20 }}>
              ...
              >
                <Text style={{ fontSize: 16, fontWeight: "bold", flex: 1 }}>{doc.name}</Text>
                <View
                  style={{
                    padding: 6,
