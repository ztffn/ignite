...
export const createDocumentSlice: StateCreator<RootStore, [], [], DocumentStore> = (set, get) => ({
  ...
  removeTag: (documentId, tag) =>
    set((state) => ({
      documents: state.documents.map((doc) =>
        doc.id === documentId ? { ...doc, tags: doc.tags.filter((t) => t !== tag) } : doc,
      ),
      selectedDocument:
        state.selectedDocument?.id === documentId
          ? {
              ...state.selectedDocument,
              tags: state.selectedDocument.tags.filter((t) => t !== tag),
            }
          : state.selectedDocument,
      error: null,
    })),

  moveDocument: (documentId, newTripId) =>
    set((state) => ({
      documents: state.documents.map((doc) =>
        doc.id === documentId ? { ...doc, tripId: newTripId } : doc,
      ),
      selectedDocument:
        state.selectedDocument?.id === documentId
