...
export const createTripSlice: StateCreator<RootStore, [], [], TripStore> = (set, get) => ({
  ...
  addCity: (tripId, city) => set((state) => ({
    ...
    selectedTrip: state.selectedTrip?.id === tripId 
      ? { ...state.selectedTrip, cities: [...state.selectedTrip.cities, city], updatedAt: new Date().toISOString() }
      : state.selectedTrip,
    error: null
  })),
  
  updateCity: (tripId, cityId, updates) => set((state) => ({
    trips: state.trips.map(trip => 
      trip.id === tripId 
        ? { 
            ...trip, 
  ...
  removeCity: (tripId, cityId) => set((state) => ({
    trips: state.trips.map(trip => 
      trip.id === tripId 
        ? { 
            ...trip, 
  ...
  setCurrentCity: (tripId, cityId) => set((state) => ({
    trips: state.trips.map(trip => 
      trip.id === tripId 
  ...
  updateWeather: (cityId, weather) => set((state) => ({
  ...
  getChallengesForTrip: (tripId) => get().challenges.filter(challenge => challenge.tripId === tripId),
});
